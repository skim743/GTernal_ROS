...
class RobotStatus(Node):
    def __init__(self, robot_id):
        ...
        self.cli = self.create_client(Status, 'GTernal' + robot_id + '/status')
        while not self.cli.wait_for_service(timeout_sec=1.0):
            self.get_logger().info('service not available, waiting again...')
        self.req = Status.Request()
...
def main():
    ...
    response = status_node.send_request()
    
    status_node.get_logger().info('Status data (batt_volt: {0}, charge_status: {1})'.format(response.batt_volt, response.charge_status))
